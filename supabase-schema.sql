...
-- 1. Novos campos em health_records (FibroVida 2.0)
ALTER TABLE health_records ADD COLUMN IF NOT EXISTS fatigue_level   INTEGER CHECK (fatigue_level >= 0 AND fatigue_level <= 10);
ALTER TABLE health_records ADD COLUMN IF NOT EXISTS energy_level    INTEGER CHECK (energy_level >= 0 AND energy_level <= 10);
ALTER TABLE health_records ADD COLUMN IF NOT EXISTS triggers        TEXT[]  DEFAULT '{}';
ALTER TABLE health_records ADD COLUMN IF NOT EXISTS body_locations  TEXT[]  DEFAULT '{}';

-- 2. Subscription / Trial em profiles
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS plan            TEXT    DEFAULT 'free' CHECK (plan IN ('free','trial','monthly','annual','lifetime','courtesy'));
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS trial_started_at TIMESTAMPTZ;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS plan_expires_at  TIMESTAMPTZ;

-- 3. Índice de performance para registros de saúde por data
CREATE INDEX IF NOT EXISTS idx_health_records_user_date ON health_records(user_id, record_date DESC);

-- 4. Push subscriptions para notificações de medicamentos
